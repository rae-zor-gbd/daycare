...
if (isset($_POST['followUpDate'])) {
  $followUpDate=strtotime(date($_POST['followUpDate']));
  if ($followUpDate!=$today) {
    $addDays=round(($followUpDate-$today)/86400);
  } else {
    $addDays=0;
  }
  $followUpDateDay=date('l', $followUpDate);
  if ($followUpDateDay=='Friday') {
    $addDays=$addDays+2;
  } elseif ($followUpDateDay=='Saturday') {
    $addDays=$addDays+1;
  }
  $sql_reminders="SELECT ownerID, ownerName, MIN(reminderDate) AS reminderDate FROM (SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, DATE_SUB(expirationDate, INTERVAL expirationWarning+$addDays DAY) AS reminderDate FROM owners o JOIN owners_packages op USING (ownerID) JOIN packages USING (packageID) WHERE expirationDate<=DATE_ADD(CURDATE(), INTERVAL expirationWarning+$addDays DAY) AND daysLeft>0 AND status!='Expired' AND ownerID NOT IN (SELECT ownerID FROM owners_packages WHERE status='Not Started') UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, DATE(NOW()) AS reminderDate FROM owners o JOIN owners_packages op USING (ownerID) JOIN packages USING (packageID) WHERE daysLeft>0 AND daysLeft<=daysLeftWarning AND status!='Expired' AND ownerID NOT IN (SELECT ownerID FROM owners_packages WHERE status='Not Started') UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder+$addDays DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE requirementStatus='Required' AND vaccineTitle!='Fecal' AND vaccineTitle!='Flu' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder+$addDays DAY) GROUP BY ownerID, ownerName UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder+$addDays DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Fecal' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder+$addDays DAY) AND ownerID NOT IN (SELECT ownerID FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Fecal' AND dueDate>=DATE_ADD(NOW(), INTERVAL sendReminder+$addDays DAY) GROUP BY ownerID) GROUP BY ownerID, ownerName UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder+$addDays DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Flu' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder+$addDays DAY) GROUP BY ownerID, ownerName) r GROUP BY ownerID, ownerName ORDER BY reminderDate DESC, ownerName";
