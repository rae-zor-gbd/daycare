...
$dateToday=date('l');
if ($dateToday=='Friday') {
  $sql_reminders="SELECT ownerID, ownerName, MIN(reminderDate) AS reminderDate FROM (SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, DATE_SUB(expirationDate, INTERVAL expirationWarning+2 DAY) AS reminderDate FROM owners o JOIN owners_packages op USING (ownerID) JOIN packages USING (packageID) WHERE expirationDate<=DATE_ADD(CURDATE(), INTERVAL expirationWarning+2 DAY) AND daysLeft>0 AND status!='Expired' AND ownerID NOT IN (SELECT ownerID FROM owners_packages WHERE status='Not Started') UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, DATE(NOW()) AS reminderDate FROM owners o JOIN owners_packages op USING (ownerID) JOIN packages USING (packageID) WHERE daysLeft>0 AND daysLeft<=daysLeftWarning AND status!='Expired' AND ownerID NOT IN (SELECT ownerID FROM owners_packages WHERE status='Not Started') UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder+2 DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE requirementStatus='Required' AND vaccineTitle!='Fecal' AND vaccineTitle!='Flu' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder+2 DAY) GROUP BY ownerID, ownerName UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder+2 DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Fecal' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder+2 DAY) AND ownerID NOT IN (SELECT ownerID FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Fecal' AND dueDate>=DATE_ADD(NOW(), INTERVAL sendReminder+2 DAY) GROUP BY ownerID) GROUP BY ownerID, ownerName UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder+2 DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Flu' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder+2 DAY) GROUP BY ownerID, ownerName) r GROUP BY ownerID, ownerName ORDER BY reminderDate DESC, ownerName";
} else {
  $sql_reminders="SELECT ownerID, ownerName, MIN(reminderDate) AS reminderDate FROM (SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, DATE_SUB(expirationDate, INTERVAL expirationWarning DAY) AS reminderDate FROM owners o JOIN owners_packages op USING (ownerID) JOIN packages USING (packageID) WHERE expirationDate<=DATE_ADD(CURDATE(), INTERVAL expirationWarning DAY) AND daysLeft>0 AND status!='Expired' AND ownerID NOT IN (SELECT ownerID FROM owners_packages WHERE status='Not Started') UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, DATE(NOW()) AS reminderDate FROM owners o JOIN owners_packages op USING (ownerID) JOIN packages USING (packageID) WHERE daysLeft>0 AND daysLeft<=daysLeftWarning AND status!='Expired' AND ownerID NOT IN (SELECT ownerID FROM owners_packages WHERE status='Not Started') UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE requirementStatus='Required' AND vaccineTitle!='Fecal' AND vaccineTitle!='Flu' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder DAY) GROUP BY ownerID, ownerName UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Fecal' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder DAY) AND ownerID NOT IN (SELECT ownerID FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Fecal' AND dueDate>=DATE_ADD(NOW(), INTERVAL sendReminder DAY) GROUP BY ownerID) GROUP BY ownerID, ownerName UNION SELECT ownerID, CONCAT(lastName, ', ', primaryOwner, IF(secondaryOwner!='' AND secondaryOwner IS NOT NULL, CONCAT(' & ', secondaryOwner), '')) AS ownerName, MIN(DATE_SUB(dueDate, INTERVAL sendReminder DAY)) AS reminderDate FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines v USING (vaccineID) JOIN owners o USING (ownerID) WHERE vaccineTitle='Flu' AND dueDate<=DATE_ADD(CURDATE(), INTERVAL sendReminder DAY) GROUP BY ownerID, ownerName) r GROUP BY ownerID, ownerName ORDER BY reminderDate DESC, ownerName";
}
...
if ($result_reminders->num_rows>0) {
  while ($row_reminders=$result_reminders->fetch_assoc()) {
    ...
    echo "<tr>
      ...
      <td>";
      if ($dateToday=='Friday') {
        $sql_vaccine_reminders="SELECT dogID, dogName, vaccineTitle, dueDate, vetName FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines vx USING (vaccineID) JOIN vets v USING (vetID) WHERE ownerID='$ownerID' AND requirementStatus='Required' AND dueDate<=DATE_ADD(NOW(), INTERVAL sendReminder+2 DAY) ORDER BY dueDate";
      } else {
        $sql_vaccine_reminders="SELECT dogID, dogName, vaccineTitle, dueDate, vetName FROM dogs d JOIN dogs_vaccines dv USING (dogID) JOIN vaccines vx USING (vaccineID) JOIN vets v USING (vetID) WHERE ownerID='$ownerID' AND requirementStatus='Required' AND dueDate<=DATE_ADD(NOW(), INTERVAL sendReminder DAY) ORDER BY dueDate";
      }
